...
export default function WaveBackground() {
  ...
  // Continuous "warp speed" streak lines radiating from center — runs forever,
  // each streak on its own independent loop so the motion never feels like it stops.
  const streaks = useMemo(() => {
    const count = 36;
    return Array.from({ length: count }, (_, i) => ({
      id: i,
      angle: (360 / count) * i + (Math.random() * 8 - 4),
      delay: Math.random() * 4,
      duration: 2.6 + Math.random() * 2.6,
      len: 55 + Math.random() * 35,
      tone: i % 3 === 0 ? "warm" : "gold",
    }));
  }, []);
  ...
  return (
    <div className="aura-bg" aria-hidden="true">
      ...
      <div className="aura-warp">
        {streaks.map((s) => (
          <span
            key={s.id}
            className={`aura-warp-streak tone-${s.tone}`}
            style={{
              "--angle": `${s.angle}deg`,
              "--delay": `${s.delay}s`,
              "--dur": `${s.duration}s`,
